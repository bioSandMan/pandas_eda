    ...
    while not stop:
        ...
        for i in range(X.shape[1]):
            if i in chosen_indices:
                ...
                max_corr = np.max(np.abs(np.corrcoef(
                    X[:, chosen_indices + [i]], rowvar=False) - np.identity(len(chosen_indices)+1)))
...
def remove_collinearity_supervised(X, y, scoring=f_classif, corr_threshold=0.4):
    ...
    for i, j in enumerate(scores):
        if len(selected_features) == 0:
            ...
            f = scores[i][0]
            max_corr = np.max(np.abs(np.corrcoef(
                X[:, selected_features + [f]], rowvar=False) - np.identity(len(selected_features)+1)))
            if max_corr < corr_threshold:
...
class SelectNonCollinear:
    ...
    def fit(self, X, y=None):
        if y is None:
            self.mask_ = remove_collinearity_unsupervised(
                X, self.correlation_threshold_)
        else:
            self.mask_ = remove_collinearity_supervised(
                X, y, self.scoring_, self.correlation_threshold_)
